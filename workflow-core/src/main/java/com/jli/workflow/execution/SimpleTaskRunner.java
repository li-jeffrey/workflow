...
public abstract class SimpleTaskRunner implements TaskRunner {
    ...
    private void fail(Integer workflowId, Throwable e) {
        WorkflowExecutor.getInstance().abortTask(new TaskAbort(workflowId, TaskStatus.FAILED, e.getMessage()));
        log.error("Error while executing task ", e);
    }

    protected void terminate(Integer workflowId, String message) {
        WorkflowExecutor.getInstance().abortTask(new TaskAbort(workflowId, TaskStatus.TERMINATED, message));
    ...
    protected void terminate(Integer workflowId, String message, Object... args) {
        ...
        WorkflowExecutor.getInstance().abortTask(new TaskAbort(workflowId, TaskStatus.TERMINATED, builtMsg));
